...
float CalcSobelMag(int3 dispatchThreadID, inout float3 c[3][3]) {
    for (int i = -1; i <= 1; ++i) {
        for (int j = -1; j <= 1; ++j) {
    ...
    */
    ...
    float3 GX = (-1.0 * c[0][0]) + (+1.0 * c[2][0]) +
                (-2.0 * c[0][1]) + (-2.0 * c[2][1]) +
                (-1.0 * c[0][2]) + (-1.0 * c[2][2]) ;
                
    float3 GY = (+1.0 * c[0][0]) + (+2.0 * c[1][0]) + (+1.0 * c[2][0]) +
                (-1.0 * c[0][2]) + (-2.0 * c[1][2]) + (-1.0 * c[2][2]) ;
                
    float3 G  = sqrt(GX*GX + GY+GY);
    float mag = saturate(CalcLuminance(G));
    return mag;
}
